...
const CardOfCrBlog = ({ title, intro_para, author, posting_date, brief_info, tags }) => {
  return (
...
function CRBlog(props_tag_name) {

  var blogs_to_show = [];
  for (var i = 0; i < blog_data.length; i++) {
    if (blog_data[i].by_cr === true)
    {
      var ele;
